...
export const analyticsController = {
  ...
  getVideoAnalytics: async (req: Request, res: Response) => {
    try {
      ...
      const uniqueViewersResult = await clickhouse.query({
        query: `
          SELECT uniqExactIf(userId, userId != '') as uniqueViewers
          FROM events_raw
          WHERE videoId = {videoId:String} AND eventType = 'VIDEO_OPENED'
        `,
        query_params: { videoId },
  ...
  getAdminDashboard: async (req: AuthRequest, res: Response) => {
    try {
      ...
      // Total videos (include legacy Video rows and new Lesson content)
      const [legacyVideosCount, readyLessonsCount] = await Promise.all([
        prisma.video.count({ where: { status: { in: ['READY', 'APPROVED'] } } }),
        prisma.lesson.count({ where: { status: 'READY' } }),
      ]);
      const totalVideos = legacyVideosCount + readyLessonsCount;
      ...
      const dailyActiveUsersResult = await clickhouse.query({
        query: `
          SELECT 
            ...
            uniqExactIf(userId, userId != '') as activeUsers
          FROM events_raw
      ...
      const watchTimePerDayResult = await clickhouse.query({
        query: `
          SELECT 
            toDate(timestamp) as date,
            sum(currentTime) / 60.0 as watchTime
          FROM events_raw
      ...
      const watchTimePerDay = (await watchTimePerDayResult.json() as Array<{
        ...
        date: d.date,
        // watchTime is now in minutes
        watchTime: Number(typeof d.watchTime === 'number' ? Number(d.watchTime) : Number(d.watchTime || 0)),
      }));
  ...
  getUserDashboard: async (req: AuthRequest, res: Response) => {
    try {
      ...
      // Videos watched - count unique content opened (cover videoId and lessonId)
      let videosWatched = 0;
      try {
        const videosWatchedResult = await clickhouse.query({
          query: `
            SELECT uniqExact(coalesce(nullIf(videoId, ''), nullIf(lessonId, ''))) as videosWatched
            FROM events_raw
            WHERE userId = {userId:String}
              AND eventType = 'VIDEO_OPENED'
              AND ( (videoId != '' AND videoId IS NOT NULL) OR (lessonId != '' AND lessonId IS NOT NULL) )
          `,
          query_params: { userId },
          format: 'JSONEachRow',
        });
      ...
      try {
        const totalWatchTimeResult = await clickhouse.query({
          query: `
            ...
            FROM (
              SELECT 
                coalesce(nullIf(videoId, ''), nullIf(lessonId, '')) as contentId,
            ...
            query: `
              ...
              FROM (
                SELECT 
                  ...
                  timestamp,
                  currentTime,
                  row_number() OVER (PARTITION BY coalesce(nullIf(videoId, ''), nullIf(lessonId, '')) ORDER BY timestamp DESC) as rn
                FROM events_raw
                WHERE userId = {userId:String} 
                  AND eventType = 'VIDEO_PROGRESS'
                  AND ( (videoId != '' AND videoId IS NOT NULL) OR (lessonId != '' AND lessonId IS NOT NULL) )
