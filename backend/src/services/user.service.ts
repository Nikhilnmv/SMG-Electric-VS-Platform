...
export class UserService {
  ...
  static generateTempPassword(length: number = 12): string {
    ...
    // Ensure minimum length to accommodate required character classes
    if (length < 4) length = 4;

    // First character should be uppercase to satisfy the "starts with uppercase" rule
    let password = uppercase[Math.floor(Math.random() * uppercase.length)];

    // Ensure at least one lowercase, one number and one special character appear
    const requiredChars = [
    ...
    const remainingCount = length - 1 - requiredChars.length;
    for (let i = 0; i < remainingCount; i++) {
      requiredChars.push(allChars[Math.floor(Math.random() * allChars.length)]);
    }

    // Shuffle the tail characters (not the first uppercase)
