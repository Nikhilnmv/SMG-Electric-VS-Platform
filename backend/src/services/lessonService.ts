...
export class LessonService {
  static async registerLessonVideo(data: LessonVideoRegistrationData) {
    ...
    if (STORAGE_MODE === 'local') {
      ...
      let fileBuffer = data.file.buffer;

      // If multer used disk storage, buffer might be undefined. Read from disk if available.
      if ((!fileBuffer || fileBuffer.length === 0) && (data.file as any).path) {
        const fs = await import('fs');
        fileBuffer = fs.readFileSync((data.file as any).path);
      }
      if (!fileBuffer || fileBuffer.length === 0) {
        throw new Error('Uploaded file has no data (buffer/path missing)');
      }
