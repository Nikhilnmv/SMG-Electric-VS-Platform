...
export class EmailService {
  ...
  private static async getTransporter(): Promise<nodemailer.Transporter | null> {
    ...
    if (env.SMTP_HOST && env.SMTP_USER && env.SMTP_PASS) {
      if (!this.transporter) {
        ...
        try {
          ...
          // If verification fails, clear transporter and continue to fallback logic
    ...
    // No SMTP configured.
    // In non-production (local dev), try a local SMTP (MailHog) at localhost:1025
    // This allows developers to run MailHog or another local SMTP server without extra config.
    if (env.NODE_ENV !== 'production') {
      try {
        ...
        await localTransport.verify();
        this.transporter = localTransport;
        console.log('[EmailService] Using local SMTP at localhost:1025');
        return this.transporter;
      } catch (err) {
        // Local SMTP not available — fall back to dev dumping/logging
        console.warn('[EmailService] Local SMTP not available, falling back to console dump');
        return null;
      }
    }

    // Production with no SMTP configured - no transporter
    return null;
  }
  ...
  static async sendEmail(options: EmailOptions): Promise<void> {
    ...
    // Store email in dev mode or any non-production environment for inspection
    if (env.DEV_MODE || env.DEV_EMAIL_DUMP || env.NODE_ENV !== 'production') {
    ...
    // If no transporter, allow non-production environments to dump emails to console
    if (!transporter) {
      if (!env.DEV_MODE && !env.DEV_EMAIL_DUMP && env.NODE_ENV === 'production') {
...
This is an automated message. Please do not reply to this email.
  ...
  static getLastEmails(limit: number = 10): EmailOptions[] {
    if (!env.DEV_MODE && env.NODE_ENV === 'production') {
      throw new Error('This method is only available in non-production environments');
    }
