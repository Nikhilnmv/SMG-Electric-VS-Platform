...
class AnalyticsTracker {
  ...
  private async sendBatch(events: AnalyticsEvent[]): Promise<void> {
    ...
    if (!token) {
      // If there's no auth token, persist the events to failed storage so they
      // can be retried later when the user logs in. This prevents losing
      // analytics emitted before authentication.
      console.warn('No auth token available — persisting analytics batch for retry');
      try {
        const failed = this.getFailedEvents();
        const toStore = events.map(e => ({ ...e, _retryCount: 0 }));
        const merged = failed.concat(toStore).slice(-50); // keep last 50
        localStorage.setItem('analytics_failed_events', JSON.stringify(merged));
      } catch (err) {
        console.error('Failed to persist analytics batch:', err);
      }
      return;
    }
  ...
  track(event: Omit<AnalyticsEvent, 'sessionId' | 'device'>) {
    // Best-effort: load user info but DO NOT require a decoded `userId` here.
    // Events should be queued even if the SDK can't decode the token; they
    // will be persisted and retried when an auth token becomes available.
    this.loadUserInfo();
