...
export default function ChangePasswordPage() {
  ...
  const validatePassword = (pw: string) => {
    const errors: string[] = [];
    if (pw.length < passwordPolicy.minLength) errors.push(`At least ${passwordPolicy.minLength} characters`);
    if (!passwordPolicy.startsWithUppercase.test(pw)) errors.push('Start with an uppercase letter');
    if (!passwordPolicy.hasLowercase.test(pw)) errors.push('Contain a lowercase letter');
    if (!passwordPolicy.hasNumber.test(pw)) errors.push('Contain a number');
    if (!passwordPolicy.hasSpecial.test(pw)) errors.push('Contain a special character (!@#$%^&*)');
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <button
            ...
            disabled={
              ...
              !validatePassword(newPassword).ok ||
              !currentPassword
            }
