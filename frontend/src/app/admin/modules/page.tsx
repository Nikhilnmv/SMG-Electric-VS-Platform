...
export default function AdminModulesPage() {
  ...
  const [lessonStatusMap, setLessonStatusMap] = useState<Record<string, string>>({});
  ...
  const fetchModules = async () => {
    try {
      ...
      // Build status map and detect newly READY lessons
      const nextStatusMap: Record<string, string> = {};
      modulesData.forEach((mod) => {
        // @ts-expect-error lessons shape from API
        (mod.lessons || []).forEach((lesson: any) => {
          nextStatusMap[lesson.id] = lesson.status;
          const prevStatus = lessonStatusMap[lesson.id];
          if (
            prevStatus &&
            prevStatus !== 'READY' &&
            lesson.status === 'READY'
          ) {
            showNotification('success', `Lesson "${lesson.title}" is now ready to watch.`);
          }
        });
      });
      setLessonStatusMap(nextStatusMap);
    } catch (err: any) {
  ...
  // Poll backend while any lesson is PROCESSING so status flips to READY without manual refresh
  useEffect(() => {
    const hasProcessing = modules.some((mod: any) =>
      (mod.lessons || []).some((lesson: any) => lesson.status === 'PROCESSING')
    );
  ...
  const handleUploadVideo = async (lessonId: string, formData: FormData) => {
    try {
      ...
      setUploadProgress(0);
      await lessonApi.uploadLessonVideo(lessonId, formData, (percent) => setUploadProgress(percent));
  ...
  // Filter modules based on search query, date filter, and category filter
  const filteredModules = modules.filter((module) => {
    ...
    // Category filter - check if module's allowedCategories includes the selected category
    const matchesCategory = categoryFilter === '' || 
      (module.allowedCategories && module.allowedCategories.includes(categoryFilter));
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              <div className="relative">
                <button
                  ...
                  className={`flex items-center gap-2 px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors focus:outline-none focus:ring-2 focus:ring-[#0B214A] ${
                    (dateFilter || categoryFilter) ? 'bg-[#0B214A] text-white border-[#0B214A] hover:bg-[#0a1a3a]' : 'bg-white text-gray-700'
                  }`}
                >
                  <Filter className={`h-5 w-5 ${(dateFilter || categoryFilter) ? 'text-white' : 'text-gray-400'}`} />
                  <span className="font-medium">Filters</span>
                  {(dateFilter || categoryFilter) && (
                    <span className="flex items-center justify-center min-w-[20px] h-5 px-1.5 text-xs font-semibold bg-white text-[#0B214A] rounded-full">
                      {(dateFilter ? 1 : 0) + (categoryFilter ? 1 : 0)}
                    </span>
                  )}
                  <ChevronDown className={`h-4 w-4 transition-transform ${showFilterDropdown ? 'transform rotate-180' : ''} ${(dateFilter || categoryFilter) ? 'text-white' : 'text-gray-400'}`} />
                </button>
                ...
                {showFilterDropdown && (
                  <>
                    ...
                    />
                    <div className="absolute right-0 mt-2 w-80 bg-white border border-gray-200 rounded-lg shadow-lg z-20 p-4 space-y-4">
                      <div className="flex items-center justify-between">
                        ...
                        {(dateFilter || categoryFilter) && (
                          <button
                            ...
                            className="text-xs text-[#0B214A] hover:text-[#0a1a3a] font-medium"
                          >
                            Clear All
                          </button>
                        )}
                      ...
                      <div>
                        ...
                        <div className="relative">
                          <select
                            ...
                            className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#0B214A] appearance-none bg-white pr-10"
                          >
                          ...
                          {categoryFilter && (
                            <button
                              ...
                              className="absolute right-8 top-1/2 transform -translate-y-1/2 p-1 text-gray-400 hover:text-gray-600 rounded"
                      ...
                      {(dateFilter || categoryFilter) && (
                        <div className="pt-3 border-t border-gray-200">
                          <div className="flex flex-wrap gap-2">
                            {dateFilter && (
                              <span className="inline-flex items-center gap-1 px-2 py-1 text-xs font-medium bg-[#0B214A] text-white rounded-full">
                                  ...
                                  <div className="flex items-center gap-2">
                                    ...
                                    {lesson.status === 'READY' && (
                                      <Link
                                        ...
                                        className="inline-flex items-center gap-1 px-3 py-1.5 text-sm text-green-700 border border-green-300 rounded-lg hover:bg-green-50 transition-colors"
                                      >
                                        Preview
                                      </Link>
                                    )}
